...
export default function DeadlinesPage() {
  const { data, loading, reload } = useDeadlines();
  const [doneIds, setDoneIds] = useState<Record<string, boolean>>({});

  const items = useMemo(() => (data ?? []).filter((d) => d.due_date), [data]);
  ...
  const events: CalendarEvent[] = useMemo(
    () =>
      items.map((a) => ({
        ...
        title: a.title,
        date: `${a.due_date}T09:00:00`,
        kind: "deadline" as const,
        urgency: urgencyFromDays(daysUntil(a.due_date)),
      })),
    [items],
  );
      ...
      {loading ? (
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-6">
          ...
          <section className="mt-6 lg:mt-0">
            <h2 className="mb-2.5 font-mono text-xs uppercase tracking-[0.08em] text-ink-2">Upcoming</h2>
            <div className="space-y-2.5">
              {items
                .filter((a) => !doneIds[a.id] && a.status !== "done")
                .map((a) => (
                  <div
                    key={a.id}
                  ...
                  >
                    ...
                    <DeadlineChip deadline={deadlineView(a.due_date)} size="sm" />
                    <button
                      onClick={() => downloadICS({ title: a.title, date: a.due_date })}
